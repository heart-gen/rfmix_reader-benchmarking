        ...
        for population_dir in sorted(phase_dir.glob("*_populations")):
            ...
            for method_dir in sorted(population_dir.iterdir()):
                ...
                if (method_dir / "locus_metrics.json").exists():
                    ...
                    metrics_dirs.extend(
                        subdir for subdir in sorted(method_dir.iterdir()) if subdir.is_dir()
                    )

                for metrics_dir in metrics_dirs:
                    locus_metrics = load_json(metrics_dir / "locus_metrics.json")
                    segment_metrics = load_json(metrics_dir / "segment_metrics.json")
                    switch_metrics = load_json(metrics_dir / "switch_error_rate.json")
                        ...
                        row.update(
                            {
                                ...
                                "chromosome": locus_metrics.get("chromosome"),
                                "interpolation_runtime_sec": locus_metrics.get(
                    ...
                    breakpoint_path = metrics_dir / "breakpoint_error.tsv"
